...
class Agent4(Agent):
    ...
    def perform_action(self, action, map_obj, event):
        ...
        if isinstance(action, MoveForward):
            return self._perform_continuous_move(map_obj, event)
        else:
            # Para otras acciones (giros), comportamiento normal
            return super().perform_action(action, map_obj, event)

    def _perform_continuous_move(self, map_obj, event):
        """
        Realiza movimiento continuo hasta punto de decisión o final.
        """
        ...
        self.event_bus.emit(event, old_position=self.position,
...
class Agent5(Agent):
    ...
    def perform_action(self, action, map_obj, event):
        ...
        if isinstance(action, (MoveUp, MoveDown, MoveRight, MoveLeft)):
            return self._perform_continuous_move(action, map_obj, event)
        else:
            # No hay otras acciones
            return False

    def _perform_continuous_move(self, action, map_obj, event):
        """
        Realiza movimiento continuo en la dirección de la acción hasta punto de decisión o final.
        """
        current_pos = self.position
